...
{   
    ...
    if (_enablePin != 0xff)
    {
        ...
        gpio_set_dir(_enablePin, true);
        gpio_put(_enablePin, _enableInverted);
    }
}
...
void    AccelStepper::enableOutputs()
{
    ...
    gpio_init(_pin[0]);
    gpio_set_dir(_pin[0], true);
    gpio_init(_pin[1]);
    gpio_set_dir(_pin[1], true);

    if (_interface == FULL4WIRE || _interface == HALF4WIRE)
    {
        gpio_init(_pin[2]);
        gpio_set_dir(_pin[2], true);
        gpio_init(_pin[3]);
        gpio_set_dir(_pin[3], true);
    }
    else if (_interface == FULL3WIRE || _interface == HALF3WIRE)
    {
        gpio_init(_pin[2]);
